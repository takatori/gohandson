...
func parseRelSize(base int, s string) (int, error) {
	i := strings.IndexFunc(s, func(c rune) bool {
		// DONE: cが数字の場合はfalse、そうでない場合はtrueを返す。
	...
	switch s[i:] {
		// DONE: "%"が指定された場合は、baseを100%として値を計算する。
	case "%":
		return (v / base) * 100, nil
	case "px":
		return v, nil
	default:
		// DONE: "%"と"px"以外の単位が指定された場合は、ErrUnknownUnitエラーを返す。
...
func (img *Image) parseSize(s string) (sz image.Point, err) {
	// DONE: sを"x"で分割し、spに入れる。
	...
	// DONE: 高さが省略された場合は、高さは幅と同じにする。
	// そうでない場合は、"x"で分割した2番目の方をパースして高さとする。
	if len(sp) == 1 {
		sz.Y = sp[0]
	}
	
	sz.Y, err = parseRelSize(img.Bounds().Max.X, sp[1])
	if err != nil {
		err = ErrInvalidSize
		return
	}
...
func (img *Image) parseBounds(s string) (r image.Rectangle, err error) {
	...
	// DONE: "+"で1つ〜3つに分割できない場合はErrInvalidBoundsエラーを返す。
